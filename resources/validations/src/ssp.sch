<sch:schema queryBinding="xslt2"
    ...
    <xsl:function as="document-node()*"
        ...
        <xsl:variable name="profile-map">
            ...
            -->
            <profile href="{concat($baselines-base-path, '/FedRAMP_rev4_LOW-baseline_profile.xml')}"
                     level="low" />
            <profile href="{concat($baselines-base-path, '/FedRAMP_rev4_MODERATE-baseline_profile.xml')}"
                     level="moderate" />
            <profile href="{concat($baselines-base-path, '/FedRAMP_rev4_HIGH-baseline_profile.xml')}"
                     level="high" />
        </xsl:variable>

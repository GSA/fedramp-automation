<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:o="http://csrc.nist.gov/ns/oscal/1.0" xpath-default-namespace="http://csrc.nist.gov/ns/oscal/1.0">
    ...
    <xsl:template name="analysis-template" as="element()">
        ...
        <analysis>
            ...
            <reports name="{$value-set/@name}" formal-name="{$value-set/f:formal-name}" description="{$value-set/f:description}" count="{count($element)}">
                <xsl:for-each select="$ok-values">
                    <xsl:variable name="match" select="$element[@value=current()]" />
                    <report value="{current()}" count="{count($match)}"></report>
                </xsl:for-each>
            </reports>
        </analysis>
    </xsl:template>
    ...
    <sch:pattern>
        ...
        <sch:rule context="/o:system-security-plan/o:control-implementation">
            ...
            <sch:report role="positive" id="each-required-control-report" test="count($required-controls) > 0">
                The following
            ...
            <sch:assert role="error" organizational-id="section-c.3" id="incomplete-core-implemented-requirements" test="not(exists($core-missing))">
            ...
            <sch:assert role="warn" organizational-id="section-c.2" id="incomplete-all-implemented-requirements" test="not(exists($all-missing))">
            ...
            <sch:assert role="warn" organizational-id="section-c.2" id="extraneous-implemented-requirements" test="not(exists($extraneous))">
            ...
            <sch:report role="positive" id="control-implemented-requirements-stats" test="count($results/errors/error) = 0">
        ...
        <sch:rule context="/o:system-security-plan/o:control-implementation/o:implemented-requirement">
            ...
            <sch:assert role="error" organizational-id="section-c.2" id="invalid-implementation-status" test="not(exists($corrections))">
            ...
            <sch:report role="positive" id="implemented-response-points" test="exists($implemented)">
                [Section C Check 2] This SSP has implemented a statement for each of the following lettered response points for required controls:
            ...
            <sch:assert role="error" organizational-id="section-c.2" id="missing-response-points" test="not(exists($missing))">
                [Section C Check 2] This SSP has not implemented a statement for each of the following lettered response points for required controls:
        ...
        <sch:rule context="/o:system-security-plan/o:control-implementation/o:implemented-requirement/o:statement/o:description">
            <sch:assert role="warning" id="extraneous-response-description" test=". => empty()">
                Response statement
                <sch:value-of select="../@statement-id" />
                has a description not within a component. That was previously allowed, but not recommended. It will soon be syntactically invalid and deprecated.
            </sch:assert>
        </sch:rule>
        ...
        <sch:rule context="/o:system-security-plan/o:control-implementation/o:implemented-requirement/o:statement/o:remarks">
            <sch:assert role="warning" id="extraneous-response-remarks" test=". => empty()">
                Response statement
                <sch:value-of select="../@statement-id" />
                has remarks not within a component. That was previously allowed, but not recommended. It will soon be syntactically invalid and deprecated.
            </sch:assert>
        </sch:rule>
        ...
        <sch:rule context="/o:system-security-plan/o:control-implementation/o:implemented-requirement/o:statement/o:by-component">
            ...
            <sch:assert role="warning" organizational-id="section-d" id="invalid-component-match" test="/o:system-security-plan/o:system-implementation/o:component[@uuid = $component-ref] => exists()">
                [Section D Checks] Response statment
                <sch:value-of select="../@statement-id" />
            ...
            <sch:assert role="error" organizational-id="section-d" id="missing-component-description" test="./o:description => exists()">
                ...
                <sch:value-of select="../@statement-id" />
                has a component, but that component is missing a required description node.
            </sch:assert>
            "
        </sch:rule>
        ...
        <sch:rule context="/o:system-security-plan/o:metadata">
            ...
            <sch:assert organizational-id="section-c.6" id="incorrect-role-association" test="not(exists($extraneous-roles))">
            ...
            <sch:assert organizational-id="section-c.6" id="incorrect-party-association" test="not(exists($extraneous-parties))">
                [Section C Check 2] This SSP has defined a responsible party with

...
<sch:pattern id="a-4-control-statistics">
    <sch:rule context="system-security-plan/control-implementation">
        ...
        <sch:assert id="threshold-count-exceeds" test="count($statuses-planned) + count($statuses-na) &lt; $percent-of-total-control-status-threshold"> There are too many controls implemented in status of planned: <sch:value-of select="count($statuses-planned)"/> and status of not-applicable: <sch:value-of select="count($statuses-na)"/> (total: <sch:value-of select="count($statuses-planned) + count($statuses-na)"/>) )when the threshold is no more than <sch:value-of select="$percent-of-total-control-status-threshold"/>.</sch:assert>
        ...
        <sch:report test="true()" id="partial-count">I see <sch:value-of select="count($statuses-partial)"/> control-implementation(s) with a status of partial.</sch:report>
        <sch:report test="true()" id="planned-count">I see <sch:value-of select="count($statuses-planned)"/> control-implementation(s) with a status of planned.</sch:report>
        <sch:report test="true()" id="total-count">I see <sch:value-of select="count($statuses-all)"/> control-implementation(s) total.</sch:report>
        <sch:report test="true()" id="threshold-count">The threshold is <sch:value-of select="$percent-of-total-control-status-threshold"/> (25% of total control-implementation(s)).</sch:report>
    </sch:rule>
</sch:pattern>

<sch:pattern id="a-4-control-statistics-test">

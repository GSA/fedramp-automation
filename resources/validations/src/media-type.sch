...
<sch:schema queryBinding="xslt2"
    ...
    <sch:pattern>
        <sch:rule context="rlink | base64"
                  role="error">
            <sch:let name="media-types-url"
                     value="'media-types.xml'" />
            <sch:assert test="doc-available($media-types-url)">media-types document is available.</sch:assert>
            <sch:let name="media-types"
                     value="doc($media-types-url)" />
            <sch:let name="media-types"
                     value="$media-types//value-set[@name = 'media-type']//enum/@value" />
            <sch:assert test="count($media-types) &gt; 1">media-types have been correctly obtained.</sch:assert>
